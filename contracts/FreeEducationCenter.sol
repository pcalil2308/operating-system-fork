...
pragma solidity >=0.8.2 <0.9.0; // Pragma remains the same, good choice
...
contract FreeEducationCenter {
    ...
    function addContent(
        ...
        require(bytes(_description).length > 0 && bytes(_description).length < 500, "FEC: Description must be between 1 and 499 characters");
